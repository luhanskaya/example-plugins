...
if( !class_exists( 'MV_Testimonials' ) ){
    ...
    class MV_Testimonials{
        ...
        public function __construct() {
            ...
            require_once( MV_TESTIMONIALS_PATH . 'widgets/class.mv-testimonials-widgets.php' );
            $MVTestimonialsWidget = new MV_Testimonials_Widget();

            add_filter( 'archive_template', array( $this, 'load_custom_template' ) );
        ...
        public function define_constants(){
            ...
            define ( 'MV_TESTIMONIALS_OVERRIDE_PATH_DIR', get_stylesheet_directory() );
        }

        public function load_custom_template( $tpl ){
            if( current_theme_supports( 'mv-testimonials' ) ){
                if( is_post_type_archive( 'mv-testimonials' ) ){
                    $tpl = $this->get_template_part_location('archive');
                } elseif (is_singular( 'mv-testimonials')) {
                    $tpl = $this->get_template_part_location('single');
                }
            }
            return $tpl;
        }

        public function get_template_part_location( $type )
        {
            $name_template = $type . '-mv-testimonials.php';
            $path = MV_TESTIMONIALS_OVERRIDE_PATH_DIR . '/templates/' . $type . '-mv-testimonials/' . $name_template;

            if(file_exists($path)) {
                $file = $path;
            } else {
                $file = MV_TESTIMONIALS_PATH . 'views/templates/' . $name_template;
            }
